 ...
 */
async function handleBan(ctx: CommandContext, context: Context<'issue_comment.created'>) {
  const [targetUser, ...reasonParts] = ctx.args;
 ...
 */
async function handleTempBan(ctx: CommandContext, context: Context<'issue_comment.created'>) {
  const [targetUser, daysStr, ...reasonParts] = ctx.args;
 ...
 */
async function handleUnban(ctx: CommandContext, context: Context<'issue_comment.created'>) {
  const [targetUser] = ctx.args;
 ...
 */
async function handleWhitelist(ctx: CommandContext, context: Context<'issue_comment.created'>) {
 ...
 */
async function handleUnwhitelist(ctx: CommandContext, context: Context<'issue_comment.created'>) {
  if (!isMaintainer(ctx.commenter, ctx.owner)) {
    await context.octokit.issues.createComment(
      context.issue({
